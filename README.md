...
- 防逆向/防伪
  - `防逆向/防伪`：关闭 / 签名防伪 / 封装延迟逆向
  - 建议：开源项目推荐使用“封装延迟逆向”，载荷不可读且可校验真伪（需后台解码）
- 接口保护
  - `启用接口签名`：为前端 `ping/copy_log` 请求增加签名校验，降低被恶意刷日志与资源消耗风险
- 前端脚本
  - `内联前端 JS（避免外链被拦截）`：将 `stegomark.js` 直接内联输出到页面，避免浏览器插件拦截外链 JS 导致前端功能失效（若用户完全禁用 JS 仍无解）
- 额外容器注入（隐藏水印）
  - `启用：额外容器隐藏水印注入`：将隐藏水印注入到你指定的前台其他容器
  - `额外容器作用范围`：仅文章/页面单页 或 全站前台页面
  - `额外容器选择器`：一行一个 CSS 选择器（例如 `.site-title` / `.sidebar` / `footer`）
  - `额外容器最小文本长度`：避免短文本“肉眼可感知”
  - `额外容器每页最多注入次数`：限制全站模式下的注入规模
- 视觉微水印透明度
